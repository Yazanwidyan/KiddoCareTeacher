...
function SelectTrigger({
  ...
  return (
    <SelectPrimitive.Trigger
      ref={ref}
      className={cn(
        'flex h-10 flex-row items-center justify-between gap-2 rounded-xl border border-input bg-background px-3 py-2 dark:bg-input/30 dark:active:bg-input/50 sm:h-9',
        Platform.select({
      ...
      {...props}>
      <>{children}</>
      <Icon as={ChevronDown} aria-hidden={true} className="size-6 text-white" />
    </SelectPrimitive.Trigger>
  );
}
          ...
          <TextClassContext.Provider value="text-popover-foreground">
            <NativeOnlyAnimatedView className="z-50" entering={FadeIn} exiting={FadeOut}>
              <SelectPrimitive.Content
                className={cn(
                  'relative z-50 min-w-[8rem] rounded-xl border border-border bg-popover',
                  Platform.select({
                    web: cn(
