...
function SelectTrigger({
  ...
  return (
    <SelectPrimitive.Trigger
      ref={ref}
      className={cn(
        'flex h-10 flex-row items-center justify-between gap-2 rounded-xl border-input bg-background px-3 py-2 dark:bg-input/30 dark:active:bg-input/50 sm:h-9',
        Platform.select({
      ...
      {...props}>
      <>{children}</>
      <Icon as={ChevronDown} aria-hidden={true} className="size-6 text-black" />
    </SelectPrimitive.Trigger>
  );
}
